...
from scripts.errorprone.Util import DataReader, ErrorproneMsg, CustomEncoder, \
...
def parse_errorprone_output(proj, report):
    pattern_raw_message = re.compile("^(.*\.java):([0-9]+): (warning|error): \[([a-zA-Z]+)] (.*)")
...
if __name__ == '__main__':
    ...
    location_to_data = "/home/yuliia/PycharmProjects/masters/results/fix/ep_output"
    ...
    time_stamp = ''
    parsed_output_file_name = os.path.join(PATH_TO_RESULTS, "fix", "ep_parsed" + time_stamp + ".json")
